...
function chatbot_ultra_upload_file_to_assistant() {
    ...
    if (empty($api_key)) {
        ...
        $response = array(
            'status' => 'error',
            'message' => 'Oops! API key is missing. Please enter your API key in the Chatbot Ultra settings.'
        );
        return $response;
    }
    ...
    if ($http_status != 200 || isset($responseData['error'])) {
        ...
        // chatbot_ultra_back_trace( 'SUCCESS', 'asst_file_id ' . $responseData['id'] );

        // Set the transient for the file id
        set_chatbot_ultra_transients('asst_file_id', $responseData['id']);
