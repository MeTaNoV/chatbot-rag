<?php
/**
 * Chatbot Ultra for WordPress - Settings - API/Model Page
 *
...
function chatbot_ultra_api_model_section_callback($args) {
    ?>
    <p>Configure the settings for the Chatbot Ultra plugin by adding your API key and selecting the GPT model of your choice.</p>
...
function chatbot_ultra_api_key_callback($args) {
    $api_key = get_option('chatbot_ultra_api_key');
    ?>
    <input type="password" id="chatbot_ultra_api_key" name="chatbot_ultra_api_key" value="<?php echo esc_attr( $api_key ); ?>" class="regular-text">
    <?php
}
...
// Model choice
function chatbot_ultra_model_choice_callback($args) {
    // Get the saved chatbot_ultra_model_choice value or default to "gpt-3.5-turbo"
    $model_choice = esc_attr(get_option('chatbot_ultra_model_choice', 'gpt-3.5-turbo'));
    // if ($model_choice == 'gpt-4-1106-preview') {
    //     $model_choice = 'gpt-4-turbo';
    // }
    ?>
    <select id="chatbot_ultra_model_choice" name="chatbot_ultra_model_choice">
...
function chatgpt_max_tokens_callback($args) {
    // Get the saved chatbot_ultra_max_tokens or default to 2000
    $chatbot_ultra_max_tokens = esc_attr(get_option('chatbot_ultra_max_tokens', '2000'));
    // Allow for a range of tokens between 100 and 4096 in 50 step increments - Ver 1.6.1
    ?>
    <select id="chatbot_ultra_max_tokens" name="chatbot_ultra_max_tokens">
        <?php
        for ($i=100; $i<=4096; $i+=50) {
            echo '<option value="' . esc_attr($i) . '" ' . selected($chatbot_ultra_max_tokens, (string)$i, false) . '>' . esc_html($i) . '</option>';
        }
        ?>
    </select>
    <?php
...
function chatbot_ultra_conversation_context_callback($args) {
    // Get the value of the setting we've registered with register_setting()
    $chatbot_ultra_conversation_context = get_option('chatbot_ultra_conversation_context');

    // Check if the option has been set, if not, use a default value
    if (empty($chatbot_ultra_conversation_context)) {
        ...
        update_option('chatbot_ultra_conversation_context', $chatbot_ultra_conversation_context);
    }

    ?>
    <!-- Define the textarea field. -->
    <textarea id='chatbot_ultra_conversation_context' name='chatbot_ultra_conversation_context' rows='5' cols='50' maxlength='2500'><?php echo esc_html(stripslashes($chatbot_ultra_conversation_context)); ?></textarea>
    <?php
}
