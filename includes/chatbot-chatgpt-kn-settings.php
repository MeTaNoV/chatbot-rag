<?php
/**
 * Chatbot Ultra for WordPress - Knowledge Navigator - Settings
 *
...
global $topwords, $words, $start_url, $domain, $max_top_words, $chatbot_ultra_diagnostics, $plugin_dir_path, $results_dir_path, $no_of_items_analyzed;
$start_url = site_url();
$domain = parse_url($start_url, PHP_URL_HOST);
$max_top_words = esc_attr(get_option('chatbot_ultra_kn_maximum_top_words', 25)); // Default to 25


function chatbot_ultra_knowledge_navigator_section_callback($args) {

    // DIAG - Diagnostic - Ver 1.6.3
    // chatbot_ultra_back_trace( 'NOTICE', 'chatbot_ultra_knowledge_navigator_section_callback');

    // NUCLEAR OPTION - OVERRIDE VALUE TO NO
    // update_option('chatbot_ultra_knowledge_navigator', 'No');
    ...
    if (in_array($run_scanner, ['Now', 'Hourly', 'Daily', 'Twice Daily', 'Weekly', 'Cancel'])) {
        ...
        // DIAG - Diagnostic - Ver 1.6.3
        // chatbot_ultra_back_trace( 'NOTICE', "$run_scanner: " . $run_scanner);
        // chatbot_ultra_back_trace( 'NOTICE', "max_top_words: " . serialize($GLOBALS['max_top_words']));
        // chatbot_ultra_back_trace( 'NOTICE', "domain: " . serialize($GLOBALS['domain']));
        // chatbot_ultra_back_trace( 'NOTICE', "start_url: " . serialize($GLOBALS['start_url']));

        $no_of_items_analyzed = 0;
        update_option('no_of_items_analyzed', $no_of_items_analyzed);

        // WP Cron Scheduler - VER 1.6.2
        // chatbot_ultra_back_trace( 'NOTICE', 'BEFORE wp_clear_scheduled_hook');
        wp_clear_scheduled_hook('knowledge_navigator_scan_hook'); // Clear before rescheduling
        // chatbot_ultra_back_trace( 'NOTICE', 'AFTER wp_clear_scheduled_hook');

        if ($run_scanner === 'Cancel') {
            $run_scanner = 'No';
            update_option('chatbot_ultra_knowledge_navigator', 'No');
            update_option('chatbot_ultra_scan_interval', 'No Schedule');
        ...
        //     chatbot_ultra_kn_acquire();
            ...
            if (!wp_next_scheduled('knowledge_navigator_scan_hook')) {
                ...
                // chatbot_ultra_back_trace( 'NOTICE', 'BEFORE crawl_scehedule_event_hook');
                ...
                // chatbot_ultra_back_trace( 'NOTICE', 'AFTER crawl_scehedule_event_hook');

                // Log scan interval - Ver 1.6.3
                if ($interval === 'Now') {
                    update_option('chatbot_ultra_scan_interval', 'No Schedule');
                } else {
                    update_option('chatbot_ultra_scan_interval', $run_scanner);
                }
    ...
    <div class="wrap">
        ...
        <div style="background-color: white; border: 1px solid #ccc; padding: 10px; margin: 10px; display: inline-block;">
            ...
            <p><b>Scheduled to Run: </b><?php echo esc_attr(get_option('chatbot_ultra_scan_interval', 'No Schedule')); ?></p>
            <p><b>Status of Last Run: </b><?php echo esc_attr(get_option('chatbot_ultra_kn_status', 'In Process')); ?></p>
...
function chatbot_ultra_knowledge_navigator_callback($args) {
    $chatbot_ultra_knowledge_navigator = esc_attr(get_option('chatbot_ultra_knowledge_navigator', 'No'));
    ?>
    <select id="chatbot_ultra_knowledge_navigator" name="chatbot_ultra_knowledge_navigator">
        <option value="No" <?php selected($chatbot_ultra_knowledge_navigator, 'No'); ?>><?php echo esc_html('No'); ?></option>
        <option value="Now" <?php selected($chatbot_ultra_knowledge_navigator, 'Now'); ?>><?php echo esc_html('Now'); ?></option>
        <option value="Hourly" <?php selected($chatbot_ultra_knowledge_navigator, 'Hourly'); ?>><?php echo esc_html('Hourly'); ?></option>
        <option value="Twice Daily" <?php selected($chatbot_ultra_knowledge_navigator, 'Twice Daily'); ?>><?php echo esc_html('Twice Daily'); ?></option>
        <option value="Daily" <?php selected($chatbot_ultra_knowledge_navigator, 'Daily'); ?>><?php echo esc_html('Daily'); ?></option>
        <option value="Weekly" <?php selected($chatbot_ultra_knowledge_navigator, 'Weekly'); ?>><?php echo esc_html('Weekly'); ?></option>
        <option value="Cancel" <?php selected($chatbot_ultra_knowledge_navigator, 'Cancel'); ?>><?php echo esc_html('Cancel'); ?></option>
    </select>
    <?php
}

function chatbot_ultra_kn_maximum_top_words_callback($args) {

    // DIAG - Diagnostic - Ver 1.6.3
    // chatbot_ultra_back_trace( 'NOTICE', 'chatbot_ultra_kn_maximum_top_words_callback');

    $GLOBALS['max_top_words'] = intval(get_option('chatbot_ultra_kn_maximum_top_words', 25));
    ?>
    <select id="chatbot_ultra_kn_maximum_top_words" name="chatbot_ultra_kn_maximum_top_words">
        <?php
