<?php
/**
 * Chatbot Ultra for WordPress - Custom GPT - Ver 1.6.9
 *
...
function runTheAssistant($thread_Id, $assistantId, $context, $api_key) {
    ...
    if ($response === FALSE) {
        // DIAG - Diagnostics
        // chatbot_ultra_back_trace( 'ERROR', 'Error unable to fetch response');
    ...
    if (http_response_code() != 200) {
        // DIAG - Diagnostics
        // chatbot_ultra_back_trace( 'ERROR', 'HTTP response code: ' . http_response_code());
...
function getTheRunsStatus($thread_Id, $runId, $api_key) {
    ...
    while ($status != "completed") {
        ...
        if (array_key_exists("status", $responseArray)) {
            ...
            $status = "failed";
            // DIAG - Diagnostics
            chatbot_ultra_back_trace( 'ERROR', "Error - GPT Assistant - Step 5");
            exit;
        }

        // DIAG - Diagnostics
        // chatbot_ultra_back_trace( 'NOTICE', '$responseArray: ' . $responseArray);
...
function getTheStepsStatus($thread_Id, $runId, $api_key) {
    ...
    while (!$status) {
        ...
        if (array_key_exists("data", $responseArray) && !is_null($responseArray["data"])) {
            ...
            $status = "failed";
            // DIAG - Diagnostics
            chatbot_ultra_back_trace( 'ERROR', "Error - GPT Assistant - Step 7.");
            exit;
        }
...
function chatbot_ultra_custom_gpt_call_api($api_key, $message, $assistantId, $thread_Id, $user_id, $page_id) {

    // DIAG - Diagnostics
    // chatbot_ultra_back_trace( 'NOTICE', 'Using Assistant ID: ' . $assistantId);
    ...
    if (empty($thread_Id)) {
        // Step 1: Create an Assistant
        // chatbot_ultra_back_trace( 'NOTICE', 'Step 1: Create an Assistant');
        $assistants_response = createAnAssistant($api_key);
        // DIAG - Print the response
        // chatbot_ultra_back_trace( 'NOTICE', $assistants_response);

        // Step 2: Get The Thread ID
        // chatbot_ultra_back_trace( 'NOTICE', 'Step 2: Get The Thread ID');
        $thread_Id = $assistants_response["id"];
        // DIAG - Diagnostics
        // chatbot_ultra_back_trace( 'NOTICE', '$thread_Id ' . $thread_Id);
        // chatbot_ultra_back_trace( 'NOTICE', '$assistantId ' . $assistantId);
        // chatbot_ultra_back_trace( 'NOTICE', '$user_id ' . $user_id);
        // chatbot_ultra_back_trace( 'NOTICE', '$page_id ' . $page_id);
        set_chatbot_ultra_threads($thread_Id, $assistantId, $user_id, $page_id);
    }

    $context = "";
    $context = esc_attr(get_option('chatbot_ultra_conversation_context', 'You are a versatile, friendly, and helpful assistant designed to support me in a variety of tasks.'));
 
    // // Context History - Ver 1.6.1 - Added here for Ver 1.7.2.1
    $chatgpt_last_response = concatenateHistory('context_history');
    // // DIAG Diagnostics - Ver 1.6.1
    // // chatbot_ultra_back_trace( 'NOTICE', '$chatgpt_last_response ' . $chatgpt_last_response);
    ...
    // $chatbot_ultra_kn_conversation_context = get_option('chatbot_ultra_kn_conversation_context', '');

    // // Append prior message, then context, then Knowledge Navigator - Ver 1.6.1
    // $context = $chatgpt_last_response . ' ' . $context . ' ' . $chatbot_ultra_kn_conversation_context;

    // Step 3: Add a Message to a Thread
    // chatbot_ultra_back_trace( 'NOTICE', 'Step 3: Add a Message to a Thread');
    $prompt = $message;

    // Fetch the file id - Ver 1.7.9
    $asst_file_id = chatbot_ultra_retrieve_file_id();
    if (empty($asst_file_id)) {
        // chatbot_ultra_back_trace( 'NOTICE', 'No file to retireve');
        $assistants_response = addAMessage($thread_Id, $prompt, $context, $api_key);
    } else {
        //DIAG - Diagnostics - Ver 1.7.9
        // chatbot_ultra_back_trace( 'NOTICE', '$asst_file_id ' . $asst_file_id);
        // DIAG - Diagnostics - Ver 1.7.9
        // chatbot_ultra_back_trace( 'NOTICE', 'Step 3: Open and read the file in the Assistant');
        // DIAG - Diagnostics - Ver 1.7.9
        // chatbot_ultra_back_trace( 'NOTICE', '$prompt ' . $prompt);
    ...
    // Step 4: Run the Assistant
    // chatbot_ultra_back_trace( 'NOTICE', 'Step 4: Run the Assistant');
    ...
    if (!is_array($assistants_response) || is_string($assistants_response)) {
        chatbot_ultra_back_trace( 'ERROR', 'Invalid response format or error occurred');
    ...
    if (isset($assistants_response["id"])) {
        ...
        chatbot_ultra_back_trace( 'ERROR', '\'$runId\' key not found in response');
    ...
    // Step 5: Get the Run's Status
    // chatbot_ultra_back_trace( 'NOTICE', 'Step 5: Get the Run\'s Status');
    getTheRunsStatus($thread_Id, $runId, $api_key);

    // Step 6: Get the Run's Steps
    // chatbot_ultra_back_trace( 'NOTICE', 'Step 6: Get the Run\'s Steps');
    $assistants_response = getTheRunsSteps($thread_Id, $runId, $api_key);
    // DIAG - Print the response
    // chatbot_ultra_back_trace( 'NOTICE', $assistants_response);

    // Step 7: Get the Step's Status
    // chatbot_ultra_back_trace( 'NOTICE', 'Step 7: Get the Step\'s Status');
    getTheStepsStatus($thread_Id, $runId, $api_key);

    // Step 8: Get the Message
    // chatbot_ultra_back_trace( 'NOTICE', 'Step 8: Get the Message');
    $assistants_response = getTheMessage($thread_Id, $api_key);
    // DIAG - Print the response
    // chatbot_ultra_back_trace( 'NOTICE', '$assistants_response: ' . $assistants_response);
...
function chatbot_ultra_retrieve_file_contents() {
    ...
    // Retrieve the file
    $file_id = get_chatbot_ultra_transients( 'file_id', $user_id, $page_id);
    $file = $file_id['file_id'];

    // Delete the file
    delete_chatbot_ultra_transients( 'file_id', $user_id, $page_id);
    ...
    // DIAG - Diagnostics - Ver 1.7.9
    // chatbot_ultra_back_trace( 'NOTICE', '$file_id ' . $file);
    // chatbot_ultra_back_trace( 'NOTICE', '$file_path_and_name ' . $file_path_and_name);
    // chatbot_ultra_back_trace( 'NOTICE', '$file_contents ' . print_r($file_contents));
...
function chatbot_ultra_retrieve_file_id() {
        ...
        // Retrieve the file
        $file_id = get_chatbot_ultra_transients( 'asst_file_id', $user_id, $page_id);
        ...
        delete_chatbot_ultra_transients( 'asst_file_id', $user_id, $page_id);
...
function deleteUploadedFile($asst_file_id) {
    ...
    // Get the API key
    $apiKey = esc_attr(get_option('chatbot_ultra_api_key'));
    ...
    if ($http_status_code == 200 || $http_status_code == 204) {
        // DIAG - Diagnostics - Ver 1.7.9
        // chatbot_ultra_back_trace( 'SUCCESS', "File deleted successfully.\n");
    } else {
        // If the request was not successful, you may want to handle it differently,
        // such as logging an error or retrying the request.
        // DIAG - Diagnostics - Ver 1.7.9
        chatbot_ultra_back_trace( 'ERROR', "HTTP status code: $http_status_code\n");
        chatbot_ultra_back_trace( 'ERROR', "Response: $response\n");
    }
