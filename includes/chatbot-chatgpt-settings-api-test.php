<?php
/**
 * Chatbot Ultra for WordPress - Settings - API/Model Test
 *
...
// Test OpenAI Model for any errors - Ver 1.6.3
 function test_ultra_api($api_key) {

    // FIXME - ADD AN OPTION TO USE WITHOUT OPENAI API KEY?

    // Reset Status and Error
    update_option('chatbot_ultra_api_status','API Error Type: Status Unknown');
    ...
    if (is_wp_error($response)) {
        ...
        chatbot_ultra_back_trace( 'ERROR', $response->get_error_message());
    }

    $response_body = json_decode(wp_remote_retrieve_body($response), true);
    // DIAG - Log the response body
    // chatbot_ultra_back_trace( 'NOTICE', $response_body);
    ...
    if (isset($response_body['error'])) {
        ...
        // chatbot_ultra_back_trace( 'SUCCESS', 'chatbot_ultra_api_status' . $updated_status);
    } else {
        ...
        chatbot_ultra_back_trace( 'ERROR', $updated_status);
    }

    update_option('chatbot_ultra_api_status', $updated_status);
    $updated_status = get_option('chatbot_ultra_api_status', 'NOT SET');
    // TODO - Monitor the chatbot_ultra_api_status option for changes
    // chatbot_ultra_back_trace( 'WARNING', 'chatbot_ultra_api_status: ' . esc_html($updated_status));
...
function chatgpt_option_updated($option_name, $old_value, $new_value) {
    ...
    if ($option_name !== 'chatgpt_model_chorice') {
        return;
    } elseif ($option_name !== 'chatbot_ultra_api_key') {
        return;
    }

    // DIAG - Log Function Call
    // chatbot_ultra_back_trace( 'NOTICE, 'chatgpt_option_updated() called');

    // FIXME Retrieve the current value of the chatbot_ultra_api_status option
    $chatbot_ultra_api_status = get_option('chatbot_ultra_api_status', 'NOT SET');
    // DIAG - Log the current value of the chatbot_ultra_api_status option
    // chatbot_ultra_back_trace( 'NOTICE', $chatbot_ultra_api_status);
    
    // Check if the option updated is related to your plugin settings
    // if ($option_name === 'chatbot_ultra_model_choice' || $option_name === 'chatbot_ultra_api_key' || empty($chatbot_ultra_api_status)) {
    if ($option_name === 'chatbot_ultra_model_choice' || $option_name === 'chatbot_ultra_api_key') {
        $api_key = get_option('chatbot_ultra_api_key', 'NOT SET');

        // Call your test function
        $test_result = test_ultra_api($api_key);
        // DIAG - Log the test result
        // chatbot_ultra_back_trace( 'WARNING', '$test_result' . $test_result);        

        // DIAG - Set the option in the admin_notice function uses to display messages
        update_option('chatbot_ultra_api_status', $test_result);
