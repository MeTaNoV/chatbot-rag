<?php
/**
 * Chatbot Ultra for WordPress - Notices
 *
...
function display_option_value_admin_notice() {
    ...
    if ($chatbot_ultra_suppress_notices == 'On') {
        return;
    }

    $kn_results = get_option('chatbot_ultra_kn_results');
    if ($kn_results) {
        // Check if notice is already dismissed
        $dismiss_url = wp_nonce_url(
            add_query_arg('dismiss_ultra_notice', '1'),
            'dismiss_ultra_notice',
            '_ultra_dismiss_nonce'
        );
        ...
        // DIAG - Diagnostics
        // chatbot_ultra_back_trace( 'NOTICE', 'Knowledge Navigator Outcome: ' . $kn_results);
    }

    // FIXME - NOT CURRENTLY WORKING WITH API-TEST.PHP
    // $api_status = get_option('chatbot_ultra_api_status');
    ...
    //         // DIAG - Diagnostics
    //         // chatbot_ultra_back_trace( 'NOTICE', 'API Status: ' . $api_status);
    // }

    $kn_status = get_option('chatbot_ultra_kn_status');
    if ($kn_status === 'Never Run') {
        ...
        // DIAG - Diagnostics
        // chatbot_ultra_back_trace( 'NOTICE', '$kn_status: ' . $kn_status);
    }
...
function dismiss_ultra_notice() {
    if (isset($_GET['dismiss_ultra_notice'])) {
        delete_option('chatbot_ultra_kn_results');
    }
}
add_action('admin_init', 'dismiss_ultra_notice');
...
//         delete_option('chatbot_ultra_api_status');
//     }
// }
// add_action('admin_init', 'dismiss_ultra_notice');
