<?php
/**
 * Chatbot Ultra for WordPress - Settings - CUstom GPTs
 *
...
function chatbot_ultra_gpt_assistants_section_callback($args) {
    ?>
    ...
    <p>See <a href="?page=chatbot-ultra&tab=support">Support</a> for more details on using multiple GPT assistants.</p>
    ...
    <p><b>NOTE:</b>For best results ensure that the shortcode appears only once on the page.</p>
        ...
        <li><b>&#91;chatbot_ultra style="floating" assistant="asst_xxxxxxxxxxxxxxxxxxxxxxxx"&#93;</b> - Floating style, GPT Assistant as set in GPT Assistant Id setting</li>
        <li><b>&#91;chatbot_ultra style="embedded" assistant="asst_xxxxxxxxxxxxxxxxxxxxxxxx"&#93;</b> - Embedded style, GPT Assistant as set in GPT Assistant Id setting</li>
...
function chatbot_ultra_use_gpt_assistant_id_callback($args) {
    $use_assistant_id = esc_attr(get_option('chatbot_ultra_use_custom_gpt_assistant_id', 'No'));
    ?>
    <select id="chatbot_ultra_use_custom_gpt_assistant_id" name="chatbot_ultra_use_custom_gpt_assistant_id">
    ...
    if ($use_assistant_id == 'No') {
        update_option('chatbot_ultra_assistant_id', '');
        update_option('chatbot_ultra_assistant_id_alternate', '');
    }
}

// Allow file uploads field callback - Ver 1.7.6
function chatbot_ultra_allow_file_uploads_callback($args) {
    $allow_file_uploads = esc_attr(get_option('chatbot_ultra_allow_file_uploads', 'No'));
    ?>
    <select id="chatbot_ultra_allow_file_uploads" name="chatbot_ultra_allow_file_uploads">
...
function chatbot_ultra_assistant_id_callback($args) {
    $assistant_id = esc_attr(get_option('chatbot_ultra_assistant_id', 'Please provide the GPT Assistant Id.'));
    $use_assistant_id = esc_attr(get_option('chatbot_ultra_use_custom_gpt_assistant_id', 'No'));
    ...
    ?>
    <input type="text" id="chatbot_ultra_assistant_id" name="chatbot_ultra_assistant_id" value="<?php echo esc_attr( $assistant_id ); ?>" class="regular-text">
    <?php
}

// GPT Assistant Id field callback - Ver 1.6.7
function chatbot_ultra_assistant_id_alternate_callback($args) {
    $assistant_id_alternate = esc_attr(get_option('chatbot_ultra_assistant_id_alternate', 'Please provide the Alternate GPT Assistant Id.'));
    $use_assistant_id = esc_attr(get_option('chatbot_ultra_use_custom_gpt_assistant_id', 'No'));
    ...
    // $assistant_id = empty($assistant_id) ? 'Please provide the GPT Assistant Id.': $assistant_id;
    ?>
    <input type="text" id="chatbot_ultra_assistant_id_alternate" name="chatbot_ultra_assistant_id_alternate" value="<?php echo esc_attr( $assistant_id_alternate ); ?>" class="regular-text">
    <?php
}
