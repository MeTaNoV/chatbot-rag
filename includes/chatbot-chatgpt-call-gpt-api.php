<?php
/**
 * Chatbot Ultra for WordPress - ChatGPT API - Ver 1.6.9
 *
...
function chatbot_ultra_call_api($api_key, $message) {
    ...
    $chatbot_ultra_max_tokens = intval(esc_attr(get_option('chatbot_ultra_max_tokens', '150')));
    ...
    // DIAG Diagnostics - Ver 1.6.1
    // chatbot_ultra_back_trace( 'NOTICE', '$context: ' . $context);

    // Added Role, System, Content Static Veriable - Ver 1.6.0
    $body = array(
        'model' => $model,
        'max_tokens' => $chatbot_ultra_max_tokens,
        'temperature' => 0.0,
        'messages' => array(
    ...
    // DIAG Diagnostics - Ver 1.6.1
    // chatbot_ultra_back_trace( 'NOTICE', '$storedc: ' . $chatbot_ultra_kn_conversation_context);
    // chatbot_ultra_back_trace( 'NOTICE', '$context: ' . $context);
    // chatbot_ultra_back_trace( 'NOTICE', '$message: ' . $message);  
    ...
    $args = array(
        ...
        'method' => 'POST',
        'data_format' => 'body',
        'timeout' => 300, // Increase the timeout values to 15 seconds to wait just a bit longer for a response from the engine
    );

    $response = wp_remote_post($api_url, $args);
    // DIAG - Diagnostics - Ver 1.6.7
    // chatbot_ultra_back_trace( 'NOTICE', '$response: ' . $response);
